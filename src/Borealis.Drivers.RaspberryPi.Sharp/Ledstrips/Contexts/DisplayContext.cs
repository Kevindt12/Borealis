...
public class DisplayContext : IDisposable, IAsyncDisposable
{
	private readonly List<DisplayState> _ledstrips;


	public DisplayContext()
	{
		_ledstrips = new List<DisplayState>();
	}
	...
	/// <returns> The <see cref="DisplayState" /> or null if there is none found. </returns>
	public virtual DisplayState? GetLedstripStateById(Guid id)
	{
	...
	/// </summary>
	/// <returns> </returns>
	public virtual IEnumerable<DisplayState> GetLedstripStates()
	{
	...
	public virtual void LoadLedstrips(IEnumerable<DisplayState> ledstrips)
	{
		...
		foreach (DisplayState ledstripState in ledstrips)
		{
	...
	protected virtual void Dispose(bool disposing)
	{
		...
		{
			foreach (DisplayState ledstripDriverState in _ledstrips.Where(x => x.HasAnimation()))
			{
	...
	protected virtual async ValueTask DisposeAsyncCore()
	{
		if (_ledstrips.Any(x => x.HasAnimation()))
		{
			foreach (DisplayState ledstripState in _ledstrips.Where(x => x.HasAnimation()))
			{
